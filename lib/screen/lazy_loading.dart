  ...
  void fetchHistoryData() {
    ...
    dates.forEachIndexed((index, element) {
      if(setListInGroup.containsKey(element.toString().substring(0, 10)))
      {
            ...
            setState(() {
              for (int j = 0 ; j < setListInGroup.entries.toList()[i].value.length; j++) {
                result.add({
                  ...
                  "name": setListInGroup.entries.toList()[i].value
                      .toList()[j]['name'],
                  "bodypart": setListInGroup.entries.toList()[i].value
                      .toList()[j]['bodypart']
                      .toString(),
                  "datediff": setListInGroup.entries.toList()[i].value
                      .toList()[j]['datediff'],
                  "count": setListInGroup.entries.toList()[i].value
                      .toList()[j]['count'],
                  "minimum_weight": setListInGroup.entries.toList()[i].value
                      .toList()[j]['minimum_weight'],
                  "maximum_weight": setListInGroup.entries.toList()[i].value
                      .toList()[j]['maximum_weight'],
                  "average_weight": setListInGroup.entries.toList()[i].value
                      .toList()[j]['average_weight'],
                  "minimum_reps": setListInGroup.entries.toList()[i].value
                      .toList()[j]['minimum_reps'],
                  "maximum_reps": setListInGroup.entries.toList()[i].value
                      .toList()[j]['maximum_reps'],
                  "average_reps": setListInGroup.entries.toList()[i].value
                      .toList()[j]['average_reps'],
                  "volumn": setListInGroup.entries.toList()[i].value
                      .toList()[j]['volumn'],
                });
              }
            });
        ...
        result.add({
          "date": element.toString().substring(0, 10),
          "datediff": DateTime.now().difference(DateTime.parse(element.toIso8601String())).inDays
        });
      }
    });
  ...
  List<DataRow> _getRows(int index) {
    ...
    for (int j = 0 ; j < resultInGroup.entries.toList()[index].value.length; j++) {
      ...
      dataCells.add(DataCell(Text(
          resultInGroup.entries.toList()[index].value.toList()[j]['bodypart']
              .toString(), style: _style)));
      dataCells.add(DataCell(ConstrainedBox(
        constraints: BoxConstraints(maxWidth: 120),
        child: Text(
            resultInGroup.entries.toList()[index].value.toList()[j]['name']
                .toString(),
      ...
      dataCells.add(DataCell(Text(
          resultInGroup.entries.toList()[index].value.toList()[j]['maximum_weight']
              .toString() + 'kg', style: _style)));
      dataCells.add(DataCell(Text(
          resultInGroup.entries.toList()[index].value.toList()[j]['average_weight']
              .toString() + 'kg', style: _style)));
      dataCells.add(DataCell(Text(
      ...
      dataCells.add(DataCell(Text(
          resultInGroup.entries.toList()[index].value.toList()[j]['minimum_reps']
              .toString(), style: _style)));
      dataCells.add(DataCell(Text(
          resultInGroup.entries.toList()[index].value.toList()[j]['average_reps']
              .toString(), style: _style)));
      dataCells.add(DataCell(Text(
          resultInGroup.entries.toList()[index].value.toList()[j]['volumn']
              .toString() + 'kg', style: _style)));
